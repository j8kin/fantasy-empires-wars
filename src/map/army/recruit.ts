...
export const recruitHero = (unit: Unit, tile: HexTileState): void => {
  tile.army.units.push({ unit: unit, count: 1 });
};

export const recruitWarriors = (unit: Unit, tile: HexTileState): void => {
  tile.army.units.push({ unit: unit, count: 20 }); // todo: probably based on unit type/owner/alignment count could be a different
};
