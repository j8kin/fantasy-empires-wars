...
describe('Construct Buildings', () => {
  ...
  describe('Constructing a building', () => {
    it('Build one Stronghold', () => {
      construct(gameStateStub, BuildingKind.STRONGHOLD, homeLand1);
    ...
    it('Build two Strongholds for two players, no intersection', () => {
      ...
      construct(gameStateStub, BuildingKind.STRONGHOLD, homeLand2);
    ...
    it('Build two Strongholds for two players, has intersection radius 1 no building on intersection', () => {
      ...
      nextPlayer(gameStateStub);
      expect(getTurnOwner(gameStateStub).id).toBe(player2Id);
      construct(gameStateStub, BuildingKind.STRONGHOLD, { row: 3, col: 5 });
      expectLands(getPlayer(gameStateStub, player1Id), getLandsInRadius(homeLand1));
    ...
    it('Construction cost 15% less if player has TreasureItem.CROWN_OF_DOMINION', () => {
      ...
      expect(getTurnOwner(gameStateStub).vault).toBe(
        200000 - Math.ceil(getBuildingInfo(BuildingKind.STRONGHOLD).buildCost * 0.85)
      );
    });
  });
  ...
  describe('Demolition building', () => {
    ...
    it('Build one Stronghold and one Demolition', () => {
      construct(gameStateStub, BuildingKind.STRONGHOLD, strongholdPos);
      construct(gameStateStub, BuildingKind.DEMOLITION, strongholdPos);
    ...
    it('Demolition non-stronghold', () => {
      ...
      construct(gameStateStub, BuildingKind.DEMOLITION, buildingPos);

      // stronghold is not destroyed
      expect(getLand(gameStateStub, strongholdPos).buildings[0].type).toBe(BuildingKind.STRONGHOLD);
    ...
    it('Demolition not change owner of the stronghold which is in radius 2 from destroyed stronghold', () => {
      ...
      construct(gameStateStub, BuildingKind.STRONGHOLD, strongholdPos);
      construct(gameStateStub, BuildingKind.STRONGHOLD, strongholdPos2);
      construct(gameStateStub, BuildingKind.BARRACKS, buildingPos);
      ...
      expect(getLand(gameStateStub, buildingPos).buildings[0].type).toBe(BuildingKind.BARRACKS);
    ...
    it('When stronghold destroyed land not change owner if another stronghold of the same owner is near', () => {
      expect(getTurnOwner(gameStateStub).id).toBe(player1Id);
      construct(gameStateStub, BuildingKind.STRONGHOLD, homeLand1);
      construct(gameStateStub, BuildingKind.STRONGHOLD, { row: 2, col: 5 }); // stronghold of player 1 near destroyed land

      nextPlayer(gameStateStub);
      expect(getTurnOwner(gameStateStub).id).toBe(player2Id);
      construct(gameStateStub, BuildingKind.STRONGHOLD, { row: 4, col: 5 });
  ...
  describe('Corner cases', () => {
    it('Building should not constructed if not enough money in vault', () => {
      ...
      construct(gameStateStub, BuildingKind.BARRACKS, emptyLand.mapPos);
