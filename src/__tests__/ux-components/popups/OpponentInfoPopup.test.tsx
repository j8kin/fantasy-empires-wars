...
describe('OpponentInfoPopup', () => {
  ...
  beforeEach(() => {
    ...
    gameStateStub.allPlayers[1].diplomacy[gameStateStub.allPlayers[0].id] =
      DiplomacyStatus.NO_TREATY;
    gameStateStub.allPlayers[0].diplomacy[gameStateStub.allPlayers[1].id] =
      DiplomacyStatus.NO_TREATY;
    gameStateStub.allPlayers[2].diplomacy[gameStateStub.allPlayers[0].id] =
      DiplomacyStatus.NO_TREATY;
    gameStateStub.allPlayers[0].diplomacy[gameStateStub.allPlayers[2].id] =
      DiplomacyStatus.NO_TREATY;
  });
  ...
  it('displays opponent name in header', () => {
    render(
      <ApplicationContextProvider>
        <OpponentInfoPopup opponent={gameStateStub.allPlayers[1]} screenPosition={mockPosition} />
      </ApplicationContextProvider>
    );

    expect(screen.getByText(gameStateStub.allPlayers[1].getName())).toBeInTheDocument();
  });
  ...
  it('renders player avatar with correct properties', () => {
    ...
    expect(avatar).toHaveAttribute('data-player-name', gameStateStub.allPlayers[1].getName());
    expect(avatar).toHaveAttribute('data-size', '55');
    expect(avatar).toHaveAttribute('data-shape', 'rectangle');
    expect(avatar).toHaveAttribute(
      'data-border-color',
      getPlayerColorValue(gameStateStub.allPlayers[1].color)
    );
  });
  ...
  it('displays race information', () => {
    ...
    expect(screen.getByText(gameStateStub.allPlayers[1].getRace())).toBeInTheDocument();
  });

  it('displays alignment information with correct color', () => {
    render(
      <ApplicationContextProvider>
        <OpponentInfoPopup opponent={gameStateStub.allPlayers[2]} screenPosition={mockPosition} />
      </ApplicationContextProvider>
    );

    expect(screen.getByText('Alignment:')).toBeInTheDocument();
    expect(screen.getByText(gameStateStub.allPlayers[2].getAlignment())).toBeInTheDocument();
  });
  ...
  it('displays level information', () => {
    ...
    expect(screen.getByText(gameStateStub.allPlayers[1].getLevel().toString())).toBeInTheDocument();
  });
  ...
  describe('diplomacy status display', () => {
    ...
    it('displays "Peace" status correctly', () => {
      gameStateStub.allPlayers[2].diplomacy[gameStateStub.allPlayers[0].id] = DiplomacyStatus.PEACE;
      gameStateStub.allPlayers[0].diplomacy[gameStateStub.allPlayers[2].id] = DiplomacyStatus.PEACE;
    ...
    it('displays "War" status correctly', () => {
      gameStateStub.allPlayers[2].diplomacy[gameStateStub.allPlayers[0].id] = DiplomacyStatus.WAR;
      gameStateStub.allPlayers[0].diplomacy[gameStateStub.allPlayers[2].id] = DiplomacyStatus.WAR;
  ...
  it('positions popup correctly relative to screen position', () => {
    ...
    render(
      <ApplicationContextProvider>
        <OpponentInfoPopup opponent={gameStateStub.allPlayers[1]} screenPosition={customPosition} />
      </ApplicationContextProvider>
    );
  ...
  it('works with different predefined players', () => {
    gameStateStub.allPlayers[1].diplomacy[gameStateStub.allPlayers[0].id] = DiplomacyStatus.WAR;
    gameStateStub.allPlayers[0].diplomacy[gameStateStub.allPlayers[1].id] = DiplomacyStatus.WAR;
  ...
  it('defaults to "No Treaty" when diplomacy status is not found', () => {
    gameStateStub.allPlayers[1].diplomacy = {};
