import { getArmyFigureImg, getFigureAssetPaths } from '../../assets/getArmyFigureImg';
...
describe('armyFigureManager', () => {
  ...
  describe('getFigureAssetKey – standard races', () => {
    describe('bronze: no heroes (regular units only)', () => {
      it('Human with regulars, no heroes → bronze', () => {
        expect(getArmyFigureImg(RaceName.HUMAN, [], true)).toBe('figure_human_bronze');
      });

      it('Elf with regulars, no heroes → bronze', () => {
        expect(getArmyFigureImg(RaceName.ELF, [], true)).toBe('figure_elf_bronze');
      });

      it('Dwarf with no heroes and no regulars → bronze (empty army edge case)', () => {
        expect(getArmyFigureImg(RaceName.DWARF, [], false)).toBe('figure_dwarf_bronze');
      });

      it('Orc with no heroes → bronze', () => {
        expect(getArmyFigureImg(RaceName.ORC, [], true)).toBe('figure_orc_bronze');
      });
    });
    ...
    describe('silver: heroes present, highest-level hero is melee', () => {
      it('Human army with a single melee hero → silver', () => {
        ...
        expect(getArmyFigureImg(RaceName.HUMAN, heroes, true)).toBe('figure_human_silver');
      });

      it('Elf army with a Ranger hero → silver', () => {
        const heroes = [makeHero(HeroUnitName.RANGER, 8)];
        expect(getArmyFigureImg(RaceName.ELF, heroes, true)).toBe('figure_elf_silver');
      });
      ...
      it('heroes-only army (no regulars) with melee hero → still silver', () => {
        ...
        expect(getArmyFigureImg(RaceName.DWARF, heroes, false)).toBe('figure_dwarf_silver');
      });
    });
    ...
    describe('gold: heroes present, highest-level hero is mage', () => {
      it('Human army with a single mage hero → gold', () => {
        ...
        expect(getArmyFigureImg(RaceName.HUMAN, heroes, true)).toBe('figure_human_gold');
      });

      it('Elf army with Druid hero → gold', () => {
        const heroes = [makeHero(HeroUnitName.DRUID, 9)];
        expect(getArmyFigureImg(RaceName.ELF, heroes, true)).toBe('figure_elf_gold');
      });
      ...
      it('heroes-only army with mage → still gold', () => {
        ...
        expect(getArmyFigureImg(RaceName.ORC, heroes, false)).toBe('figure_orc_gold');
      });
  ...
  describe('getFigureAssetKey – Undead (driven)', () => {
    it('creatures: regulars present, no heroes', () => {
      expect(getArmyFigureImg(RaceName.UNDEAD, [], true)).toBe('figure_driven_creatures');
    });

    it('hero: heroes present, no regulars', () => {
      const heroes = [makeHero(HeroUnitName.WARSMITH, 4)];
      expect(getArmyFigureImg(RaceName.UNDEAD, heroes, false)).toBe('figure_driven_hero');
    });
    ...
    it('control: both heroes and regulars present', () => {
      const heroes = [makeHero(HeroUnitName.WARSMITH, 4)];
      expect(getArmyFigureImg(RaceName.UNDEAD, heroes, true)).toBe('figure_driven_control');
    });

    it('creatures: no heroes and no regulars (empty army edge case)', () => {
      expect(getArmyFigureImg(RaceName.UNDEAD, [], false)).toBe('figure_driven_creatures');
    });
  });
});
