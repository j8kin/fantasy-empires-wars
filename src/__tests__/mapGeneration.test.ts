...
describe('Map Generation', () => {
  it.each([
    ...
    Object.values(lands.lands).forEach((land) => {
      expect(land.type).not.toBe(LandName.NONE);
      expect(land.goldPerTurn).toBeGreaterThan(0);
    });

    // Special lands should be generated
    getMainSpecialLandKinds().forEach((LandKind) => {
      expect(Object.values(lands.lands).some((land) => land.type === LandKind)).toBeTruthy();
      expect(Object.values(lands.lands).filter((land) => land.type === LandKind)).toHaveLength(1);
      expect(Object.values(lands.lands).some((land) => land.type === getNearSpecialLandKinds(LandKind))).toBeTruthy();
      expect(
        Object.values(lands.lands).filter((land) => land.type === getNearSpecialLandKinds(LandKind)).length
      ).toBeGreaterThan(1);
      expect(
        Object.values(lands.lands).filter((land) => land.type === getNearSpecialLandKinds(LandKind)).length
      ).toBeLessThan(6);
